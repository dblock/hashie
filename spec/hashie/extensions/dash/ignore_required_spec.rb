...
describe Hashie::Extensions::Dash::IgnoreRequired do
  context 'included in Dash' do
    ...
    it 'silently ignores required properties on initialization' do
      expect { subject.new(city: 'New York') }.not_to raise_error
    end

    it 'raises errors for undefined properties on initialization' do
      expect { subject.new(city: 'Toronto', province: 'Ontario') }
        .to raise_error(NoMethodError, /property 'province' is not defined/)
    end
    ...
    it 'allows setting a required property afterwards' do
      hash = subject.new(city: 'Toronto')
      hash.state = 'Ontario'
      expect(hash.state).to eq('Ontario')
    end
  end
  ...
  context 'combined with Coercion' do
    ...
    context 'with nested, coerced Dashes' do
      ...
      it 'does not propagate ignoring required properties to nested, coerced Dashes' do
        address = { street: 'Pennsylvania Avenue' }
        expect { subject.new(address: address) }
          .to raise_error(ArgumentError, /property 'number' is required for Address/)
      end
    end
  end
end
